	...
	protected function _export_to_excel($data)
	{
		...
		foreach($data->columns as $column){
			$phpExcel->setActiveSheetIndex(0)
				->setCellValueByColumnAndRow($i,1,$column->display_as);
			$i++;
		}
		$j=2;$i=0;
		foreach($data->list as $num_row => $row){
			foreach($data->columns as $column){
				$phpExcel->setActiveSheetIndex(0)
					->setCellValueByColumnAndRow($i,$j,$this->_trim_export_string($row->{$column->field_name}));
				$i++;
			}
			$i=0;
			$j++;
		...
		$filename="export-".date("Y-m-d_H:i:s"); //save our workbook as this file name
