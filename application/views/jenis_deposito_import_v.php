...
if(isset($header)) {
	...
	echo '
			<div class="alert alert-info">
				...
				Pastikan data sesuai dengan yang akan diimport, hindari duplikat data, cek kembali pada jenis deposito, apakah data diatas memang belum ada di database.<br>
				Klik BATAL IMPORT untuk memperbaiki data sebelum diimport.
			</div>
		';
	//$hidden = $values;
	echo form_open('jenis_deposito/import_db', '', array('val_arr' => $hidden_arr));
	echo '<input name="submit" type="submit" class="btn btn-primary" value="IMPORT DATA JENIS DEPOSITO KE DATABASE" />';
	echo ' <a href="'.site_url('jenis_deposito/import_batal').'" class="btn btn-warning">BATAL IMPORT</a>';
	echo form_close();
	$form_view = false;
}
?>
<?php if($form_view) { ?>
	...
	<input type="file" name="import_jenis_deposito" size="20" accept="application/octet-stream" />
	<br /><br />
