...
if(isset($header)) {
	...
	echo '
			<div class="alert alert-info">
				...
				Pastikan data sesuai dengan yang akan diimport, hindari duplikat data, cek kembali pada data barang, apakah data diatas memang belum ada di database.<br>
				Klik BATAL IMPORT untuk memperbaiki data sebelum diimport.
			</div>
		';
	//$hidden = $values;
	echo form_open('data_barang/import_db', '', array('val_arr' => $hidden_arr));
	echo '<input name="submit" type="submit" class="btn btn-primary" value="IMPORT DATA DATA BARANG KE DATABASE" />';
	echo ' <a href="'.site_url('data_barang/import_batal').'" class="btn btn-warning">BATAL IMPORT</a>';
	echo form_close();
	$form_view = false;
}
?>
