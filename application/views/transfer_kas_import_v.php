...
if(isset($header)) {
	...
	echo '
		...
		';
	//$hidden = $values;
	echo form_open('transfer_kas/import_db', '', array('val_arr' => $hidden_arr));
	echo '<input name="submit" type="submit" class="btn btn-primary" value="IMPORT DATA PEMASUKAN KAS TUNAI KE DATABASE" />';
	echo ' <a href="'.site_url('transfer_kas/import_batal').'" class="btn btn-warning">BATAL IMPORT</a>';
	echo form_close();
	$form_view = false;
}
?>
<?php if($form_view) { ?>
	...
	<input type="file" name="import_transfer_kas" size="20" accept="application/vnd.ms-excel, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" />
	<br /><br />
