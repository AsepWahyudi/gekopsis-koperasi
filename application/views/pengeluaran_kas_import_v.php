...
if(isset($header)) {
	...
	echo '
			<div class="alert alert-info">
				...
				Pastikan data sesuai dengan yang akan diimport, hindari duplikat data, cek kembali pada transaksi pengeluaran kas, apakah data diatas memang belum ada di database.<br>
				Klik BATAL IMPORT untuk memperbaiki data sebelum diimport.
			</div>
		';
	//$hidden = $values;
	echo form_open('pengeluaran_kas/import_db', '', array('val_arr' => $hidden_arr));
	echo '<input name="submit" type="submit" class="btn btn-primary" value="IMPORT DATA PEMASUKAN KAS TUNAI KE DATABASE" />';
	echo ' <a href="'.site_url('pengeluaran_kas/import_batal').'" class="btn btn-warning">BATAL IMPORT</a>';
	echo form_close();
	$form_view = false;
}
?>
<?php if($form_view) { ?>
	...
	<input type="file" name="import_pengeluran_kas" size="20" accept="application/vnd.ms-excel, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" />
	<br /><br />
