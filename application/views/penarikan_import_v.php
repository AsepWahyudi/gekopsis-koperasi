...
if(isset($header)) {
	...
	echo '
			<div class="alert alert-info">
				...
				Pastikan data sesuai dengan yang akan diimport, hindari duplikat data, cek kembali pada daftar penarikan tunai, apakah data diatas memang belum ada di database.<br>
				Klik BATAL IMPORT untuk memperbaiki data sebelum diimport.
			</div>
		';
	//$hidden = $values;
	echo form_open('penarikan/import_db', '', array('val_arr' => $hidden_arr));
	echo '<input name="submit" type="submit" class="btn btn-primary" value="IMPORT DATA SETORAN TUNAI KE DATABASE" />';
	echo ' <a href="'.site_url('penarikan/import_batal').'" class="btn btn-warning">BATAL IMPORT</a>';
	echo form_close();
	$form_view = false;
}
?>
<?php if($form_view) { ?>
	...
	<input type="file" name="import_penarikan" size="20" accept="application/vnd.ms-excel, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" />
	<br /><br />
