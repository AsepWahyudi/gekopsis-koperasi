...
<div class="box-body">
<p></p>
<p style="text-align:center; font-size: 15pt; font-weight: bold;"> Laporan Deposito Anggota <br> Periode <?php echo $tgl_periode_txt; ?> </p>

<table  class="table table-bordered">
	<tr class="header_kolom">
		<th style="width:5%; vertical-align: middle; text-align:center" > No. </th>
		<th style="width:35%; vertical-align: middle; text-align:center">Jenis Akun </th>
		<th style="width:20%; vertical-align: middle; text-align:center"> Deposito  </th>
	...
	foreach ($data_jns_deposito as $jenis) {
	if(($no % 2) == 0) {
		$warna="#FFFFCC"; } 
	else {
		$warna="#FFFFFF"; }
...
function cetak () {
	var tgl_dari = $('input[name=daterangepicker_start]').val();
	var tgl_samp = $('input[name=daterangepicker_end]').val();
	var win = window.open('<?php echo site_url("lap_deposito/cetak/?tgl_dari=' + tgl_dari + '&tgl_samp=' + tgl_samp + '"); ?>');
	if (win) {
		win.focus();
	} else {
...
function export_excel() {
	...
	<?php echo site_url('lap_deposito/export_excel'); ?>');
	

	var win = window.open('<?php echo site_url("lap_deposito/export_excel/?tgl_dari=' + tgl_dari + '&tgl_samp=' + tgl_samp + '"); ?>');
	if (win) {
		win.focus();
	} else {
		alert('Popup jangan di block');
