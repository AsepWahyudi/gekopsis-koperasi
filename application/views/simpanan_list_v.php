...
<thead>
	<tr>
		...
		<th data-options="field:'tenor_txt', width:'15', halign:'center', align:'right'" hidden="true">Tenor</th>
		...
		<th data-options="field:'jenis_cabang', width:'15', halign:'center', align:'left'" hidden="true">Jenis Cabang</th>
		...
		<th data-options="field:'bunga',halign:'center', align:'center'" hidden="true">bunga</th>
...
<div id="tb" style="height: 35px;">
	...
	<div class="pull-right" style="vertical-align: middle;">
		...
		<a href="<?=base_url()?>simpanan/export_excel" class="easyui-linkbutton" iconCls="icon-excel" plain="false">Ekspor</a>
	</div>
</div>
					...
					<table>
						...
						<tr style="height:35px" id="field-tenor">
							...
							<td>
								...
								<input id="tenor_txt" name ="tenor_txt" type ="hidden" style="width:190px; height:20px">
							</td>	
						</tr>
						<tr style="height:35px">
							<td>Bunga (%)</td>
							<td>:</td>
							<td>
								<input class="easyui-numberbox" id="bunga" name="bunga" class="easyui-validatebox" required="true" style="width:195px; height:25px"  />
							</td>	
						</tr>
						<tr style="height:35px">
...
$(document).ready(function() {
	$('#jenis_id').change(function(){
		...
		//var tenor_txt = $('#tenor_txt').val();
					
		//if(tenor_txt=='Y'){

		//	$('#field-tenor').show();

		//	$('#tenor option[value=0]').attr('selected','selected');
		...
		.done(function(result) {
			...
			//$('#jumlah').numberbox('setValue', data.jumlah);
			$('#jumlah').focus();
			$('#jumlah').select();	
			$('#bunga').numberbox('setValue', data.bunga);
			$('#bunga ~ span input').focus();
			$('#bunga ~ span input').select();
		})
		.fail(function() {
...
function create(){
	...
	$('#jenis_cabang option[value="0"]').prop('selected', true);
	$("#anggota_poto").html('');
...
function save() {
	...
	var jenis_cabang = $("#jenis_cabang").val();
	...
	if(jenis_cabang == 0) {
		$.messager.show({
			title:'<div><i class="fa fa-warning"></i> Peringatan ! </div>',
			msg: '<div class="text-red"><i class="fa fa-ban"></i> Maaf, Jenis Cabang belum dipilih.</div>',
			timeout:2000,
			showType:'slide'
		});
		$("#jenis_id").focus();
...
function update(){
	...
	if(row){
		if (row.is_approve === null){ 	
			...
			$.messager.show({
			title:'<div><i class="fa fa-warning"></i> Peringatan !</div>',
			msg: '<div class="text-red"><i class="fa fa-ban"></i> Data sudah di approve</div>',
			timeout:2000,
			showType:'slide'
			});
		}
				...
				if (row.is_approve !== 'X') { 
								$.messager.confirm('Konfirmasi','Apakah anda ingin approve data <code>' + row.id_txt + '</code>  ?',function(r){  
									if (r){  
										$.ajax({
											type	: "POST",
											url		: "<?php echo site_url('simpanan/approve'); ?>",
											data	: 'id='+row.id,
											success	: function(result){
												var result = eval('('+result+')');
												$.messager.show({
...
function cetak () {
	...
	var win = window.open('<?php echo site_url("simpanan/cetak_laporan/?cari_simpanan=' + cari_simpanan + '&kode_transaksi=' + kode_transaksi + '&tgl_dari=' + tgl_dari + '&tgl_sampai=' + tgl_sampai + '&cari_nama=' + cari_nama + '&cari_anggota=' + cari_anggota + '"); ?>');
	if (win) {
		win.focus();
	} else {
