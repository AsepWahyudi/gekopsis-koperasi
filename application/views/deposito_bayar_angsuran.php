...
$sisa_bayar = (((($row_pinjam->jumlah * $row_pinjam->bunga)/100) / $row_pinjam->tenor) + ($row_pinjam->jumlah / $row_pinjam->tenor))   - $dibayar;
...
<?php
	...
	echo ' <a href="'.site_url('deposito/detail').'/'.$row_pinjam->id.'" class="btn btn-sm btn-primary"><i class="fa fa-file-o"></i> Detail</a>';
?> 
...
<div class="box box-solid box-primary">
	<div class="box-header" title="Detail Deposito" data-toggle="" data-original-title="Detail Deposito">
...
title="Data Pembayaran Deposito" 
style="width:auto; height: auto;" 
url="<?php echo site_url('deposito/bayar_ajax_list') . '/' . $master_id; ?>" 
...
<script type="text/javascript">
	...
	function create() {
		...
		url = '<?php echo site_url('deposito/create_angsuran'); ?>';
		...
		$.ajax({
			type	: "POST",
			url		: "<?php echo site_url('deposito/get_ags_ke') . '/'.$master_id.''; ?>",
	...
	function update(){
		...
		if(row) {
			url = '<?php echo site_url('deposito/update_angsuran'); ?>/' + row.id;
		...
		if (row){ 
			$.messager.confirm('Konfirmasi','Apakah Anda akan menghapus data kode bayar : <code>' + row.id_txt + '</code> ?',function(r){  
				if (r){  
					$.ajax({
						type	: "POST",
						url		: "<?php echo site_url('deposito/delete_angsuran'); ?>",
