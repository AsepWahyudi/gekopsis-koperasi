			...
			<tr>
				...
				<td>
					...
					<tr>
						...
						<td> : </td>
						<td class="h_kanan"><?php echo number_format(nsi_round($row_pinjam->ags_per_bulan + $s_wajib->jumlah)); ?></td>
					</tr>
					</table>
				</td>			
			</tr>
...
<div id="dialog-form" class="easyui-dialog" show= "blind" hide= "blind" modal="true" resizable="false" style="width:400px; height:340px; padding: 20px 25px" closed="true" buttons="#dialog-buttons" style="display: none;">
	<form id="form" method="post" novalidate>
		...
		<tr>
			<td> Nomor Pinjam</td>
			<td> :</td>
			<td> <input type="text" id="pinjam_id_txt" name="pinjam_id_txt" value="<?php echo $row_pinjam->nomor_pinjaman; ?>" readonly="true" style="width:195px; height:20px">
				<input type="hidden" id="pinjam_id" name="pinjam_id" value="<?php echo  $master_id; ?>" readonly="true"  />
		</tr>
		<tr>
			<td> Sisa Tagihan</td>
			<td> :</td>
			<td><input type="text" class="inputform" id="tagihan" name="tagihan" style="width:195px; height:23px"/></td>
		</tr>
		<tr>
			<td> Denda</td>
			<td> :</td>
			<td><input id="denda_nilai" name="denda_nilai" data-options="precision:0,groupSeparator:',',decimalSeparator:','," class="easyui-numberbox" style="width:195px; height:23px"/></td>
		</tr>	
		<tr>
			<td> Bunga</td>
			<td> :</td>
			<td><input id="bunga" name="bunga" data-options="precision:0,groupSeparator:',',decimalSeparator:',',"  class="easyui-numberbox" style="width:195px; height:23px"/></td>
		</tr>
		<tr style="height:30px">
			<td> Jumlah Bayar</td>
			<td> :</td>
			<td> <input id="jumlah_bayar" name="jumlah_bayar" data-options="precision:0,groupSeparator:',',decimalSeparator:','," class = "easyui-numberbox"  required="true" style="width:201px; height:23px"/>
				</td>
		</tr>	
		<tr>
			<td> Simpan Ke Kas</td>
...
function create(){
	...
	jQuery('#pinjam_id_txt').val('<?php echo $row_pinjam->nomor_pinjaman ?>');
	...
	$('#jumlah_bayar ~ span input').keyup(function(){
		...
		$('#jumlah_bayar').val(number_format(val_jumlah));
	});

	$('#denda_nilai').keyup(function(){

		var denda = $(this).val();
		$('#denda_nilai').val(number_format(denda));
		
		//$('#jumlah_bayar').numberbox('setValue', number_format(total));
		//$('#jumlah_bayar').val('setValue', number_format(total));
	...
	$('#denda_nilai ~ span input').blur(function(){
		...
		var total=0;
		var val_jumlah = $('#tagihan').val();
		val_jumlah = val_jumlah.replace(/,/g,'');
		total = parseFloat(val_jumlah) + parseFloat(denda);
		$('#denda_nilai').numberbox('setValue', denda);
		$('#jumlah_bayar').numberbox('setValue', total);
	...
	$('#bunga  ~ span input').blur(function(){
		var denda = $('#denda_nilai').val();
		var bunga = $('#bunga').val();
		var total=0;
		var val_jumlah = $('#tagihan').val();
		val_jumlah = val_jumlah.replace(/,/g,'');
		total = parseFloat(val_jumlah) + parseFloat(denda);
		vjml_bayar = parseFloat(total) + parseFloat(((val_jumlah * bunga ) / 100));
		$('#jumlah_bayar').numberbox('setValue', vjml_bayar);
	});
