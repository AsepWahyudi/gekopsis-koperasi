...
class kategori_asset extends AdminController {
	...
	public function index() {
		...
		$this->data['judul_sub'] = 'Kategori Asset <a href="'.site_url('kategori_asset/import').'" class="btn btn-sm btn-success">Import Data</a>';

		$this->output->set_template('gc');

		$this->load->library('grocery_CRUD');
		$crud = new grocery_CRUD();
		$crud->set_table('kategori_asset','accpenyusutan','accakumulasipenyusutan');
		$crud->set_subject('kategori_asset');
	
		$crud->fields('kategori_asset','accpenyusutan','accakumulasipenyusutan');		
		$crud->required_fields('kategori_asset','accpenyusutan','accakumulasipenyusutan');
		$crud->unique_fields(array('kategori_asset'));

		$crud->display_as('accpenyusutan','Akun Penyusutan');
		$crud->display_as('accakumulasipenyusutan','Akun Akumulasi Penyusutan');
	...
	function import() {
		...
		$this->data['judul_sub'] = 'Kategori Asset <a href="'.site_url('kategori_asset').'" class="btn btn-sm btn-success">Kembali</a>';
		...
		if($this->input->post('submit')) {
			...
			if ( ! $this->upload->do_upload('import_kategori_asset')) {
		...
		$this->data['isi'] = $this->load->view('kategori_asset_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);
	}
	
	function import_db() {
		if($this->input->post('submit')) {
			$this->load->model('kategori_asset_m','kategori_asset', TRUE);
			$data_import = $this->input->post('val_arr');
			if($this->kategori_asset->import_db($data_import)) {
			...
			redirect('kategori_asset/import');
		} else {
			$this->session->set_flashdata('import', 'NO');
			redirect('kategori_asset/import');
		}
