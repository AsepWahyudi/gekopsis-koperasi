...
class Jenis_kas extends AdminController {
	...
	public function index() {
		...
		$this->data['judul_sub'] = 'Jenis Kas <a href="'.site_url('jenis_kas/import').'" class="btn btn-sm btn-success">Import Data</a>';
		...
		$crud->columns('nama', 'jns_akun_id', 'aktif', 'tmpl_simpan', 'tmpl_penarikan', 'tmpl_pinjaman', 'tmpl_bayar','tmpl_pemasukan','tmpl_pengeluaran','tmpl_transfer');
		$crud->fields('nama', 'jns_akun_id', 'aktif', 'tmpl_simpan', 'tmpl_penarikan', 'tmpl_pinjaman', 'tmpl_bayar','tmpl_pemasukan','tmpl_pengeluaran','tmpl_transfer');
		...
		$crud->display_as('jns_akun_id','Akun');
		$crud->required_fields('nama','jns_akun_id');
		$crud->set_relation('jns_akun_id','jns_akun','{no_akun} - {nama_akun}',"kelompok_laporan = 'NERACA' and jenis_akun = 'SUB AKUN'");
	...
	function import() {
		...
		$this->data['judul_sub'] = 'Jenis Kas <a href="'.site_url('jenis_kas').'" class="btn btn-sm btn-success">Kembali</a>';
		...
		if($this->input->post('submit')) {
			...
			if ( ! $this->upload->do_upload('import_jenis_kas')) {
		...
		$this->data['isi'] = $this->load->view('jenis_kas_import_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);
	}
	
	function import_db() {
		if($this->input->post('submit')) {
			$this->load->model('jenis_kas_m','jenis_kas', TRUE);
			$data_import = $this->input->post('val_arr');
			if($this->jenis_kas->import_db($data_import)) {
