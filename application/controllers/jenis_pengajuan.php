...
class Jenis_pengajuan extends AdminController {
	...
	public function index() {
		...
		$this->data['judul_sub'] = 'Jenis Pengajuan <a href="'.site_url('jenis_pengajuan/import').'" class="btn btn-sm btn-success">Import Data</a>';

		$this->output->set_template('gc');

		$this->load->library('grocery_CRUD');
		$crud = new grocery_CRUD();
		$crud->set_table('jns_pengajuan');
		$crud->set_subject('Jenis Pengajuan');
	
		//$crud->fields('jns_trans','pemasukan','pengeluaran','aktif');
		$crud->fields('jenis_pengajuan','fix_angsuran','lama_angsuran','inisial_id');
		$crud->columns('jenis_pengajuan','fix_angsuran','lama_angsuran','inisial_id');
		
		$crud->required_fields('jenis_pengajuan','fix_angsuran','lama_angsuran','inisial_id');
	...
	function import() {
		...
		$this->data['judul_sub'] = 'Jenis Pengajuan <a href="'.site_url('jenis_pengajuan').'" class="btn btn-sm btn-success">Kembali</a>';
		...
		if($this->input->post('submit')) {
			...
			if ( ! $this->upload->do_upload('import_jenis_pengajuan')) {
		...
		$this->data['isi'] = $this->load->view('jenis_pengajuan_import_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);
	}
	
	function import_db() {
		if($this->input->post('submit')) {
			$this->load->model('jenis_pengajuan_m','jenis_pengajuan', TRUE);
			$data_import = $this->input->post('val_arr');
			if($this->jenis_pengajuan->import_db($data_import)) {
			...
			redirect('jenis_pengajuan/import');
		} else {
			$this->session->set_flashdata('import', 'NO');
			redirect('jenis_pengajuan/import');
		}
