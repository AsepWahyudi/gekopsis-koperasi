...
class Jenis_anggota extends AdminController {
	...
	public function index() {
		...
		$this->data['judul_sub'] = 'Jenis Anggota <a href="'.site_url('jenis_anggota/import').'" class="btn btn-sm btn-success">Import Data</a>';

		$this->output->set_template('gc');

		$this->load->library('grocery_CRUD');
		$crud = new grocery_CRUD();
		$crud->set_table('jns_anggota');
		$crud->set_subject('Jenis Anggota');
	
		$crud->fields('kode','nama','status');		
		$crud->required_fields('kode','nama','status');
		$crud->unique_fields(array('kode','nama'));
	...
	function import() {
		...
		$this->data['judul_sub'] = 'Jenis Anggota <a href="'.site_url('jenis_anggota').'" class="btn btn-sm btn-success">Kembali</a>';
		...
		if($this->input->post('submit')) {
			...
			if ( ! $this->upload->do_upload('import_jenis_anggota')) {
		...
		$this->data['isi'] = $this->load->view('jenis_anggota_import_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);
	}
	
	function import_db() {
		if($this->input->post('submit')) {
			$this->load->model('jenis_anggota_m','jenis_anggota', TRUE);
			$data_import = $this->input->post('val_arr');
			if($this->jenis_anggota->import_db($data_import)) {
