...
class Jenis_angsuran extends AdminController {
	...
	public function index() {
		...
		$this->data['judul_sub'] = 'Jenis Angsuran <a href="'.site_url('jenis_angsuran/import').'" class="btn btn-sm btn-success">Import Data</a>';
		...
		$crud->display_as('ket','Lama Angsuran (Hari/Minggu/Bulan)');
	...
	function import() {
		...
		$this->data['judul_sub'] = 'Jenis Angsuran <a href="'.site_url('jenis_angsuran').'" class="btn btn-sm btn-success">Kembali</a>';
		...
		if($this->input->post('submit')) {
			...
			if ( ! $this->upload->do_upload('import_jenis_angsuran')) {
						...
						for ($row = 1; $row <= $highestRow; ++$row) {
						   //echo '<tr>';
							for ($col = 0; $col < $highestColumnIndex; ++$col) {
								...
								if($row === 1) {
									if($kolom == 'A') {
										$header[$kolom] = 'Lama Angsuran (Bulan)';
									} else {
										$header[$kolom] = $val;
									}
								} else {
		...
		$this->data['isi'] = $this->load->view('jenis_angsuran_import_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);
	}
	
	function import_db() {
		if($this->input->post('submit')) {
			$this->load->model('jenis_angsuran_m','jenis_angsuran', TRUE);
			$data_import = $this->input->post('val_arr');
			if($this->jenis_angsuran->import_db($data_import)) {
			...
			redirect('jenis_angsuran/import');
		} else {
			$this->session->set_flashdata('import', 'NO');
			redirect('jenis_angsuran/import');
		}
