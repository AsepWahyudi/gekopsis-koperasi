...
class Tarik_deposito extends OperatorController {
	
	public function __construct() {
		parent::__construct();	
		$this->load->helper('fungsi');
		$this->load->model('deposito_m');
		$this->load->model('deposito_penarikan_m');
	...
	public function index() {
		...
		$this->data['judul_sub'] = 'Penarikan Deposito <a href="'.site_url('penarikan/import').'" class="btn btn-sm btn-success">Import Data</a>';
		...
		$this->data['isi'] = $this->load->view('deposito_penarikan_list_v', $this->data, TRUE);
	...
	function get_jml_tabungan() {
		$id = isset($_POST['anggota_id']) ? $_POST['anggota_id'] : '';
		$jml= $this->deposito_penarikan_m->get_jml_simpanan($id);
	...
	function ajax_list() {
		...
		$data   = $this->deposito_penarikan_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		foreach ($data['data'] as $r) {
			...
			$nama_simpanan = $this->deposito_penarikan_m->get_jenis_simpan($r->jenis_id); 
			...
			$rows[$i]['jenis_id_txt'] =$nama_simpanan->jns_deposito;
	...
	public function create() {
		...
		if($this->deposito_penarikan_m->create()){
	...
	public function approve() {
		...
		if($this->deposito_penarikan_m->approve($id)) {
	...
	public function update($id=null) {
		...
		if($this->deposito_penarikan_m->update($id)) {
	...
	public function delete() {
		...
		$id = intval(addslashes($_POST['id']));
		if($this->deposito_penarikan_m->delete($id)) {
	...
	function cetak_laporan() {
		$penarikan = $this->deposito_penarikan_m->lap_data_penarikan();
		if($penarikan == FALSE) {
			//redirect('penarikan');
		...
		<table width="100%" cellspacing="0" cellpadding="3" border="1" border-collapse= "collapse">
			...
			foreach ($penarikan as $row) {
				$anggota= $this->deposito_penarikan_m->get_data_anggota($row->anggota_id);
				$jns_simpan= $this->deposito_penarikan_m->get_jenis_simpan($row->jenis_id);
	...
	function import_db() {
		if($this->input->post('submit')) {
			...
			if($this->deposito_penarikan_m->import_db($data_import)) {
	...
	function export_excel(){
		...
		$data   = $this->deposito_penarikan_m->get_data_excel();
		$i	= 0;
		$rows   = array(); 
