...
class Simpanan extends OperatorController {
	...
	function ajax_list() {
		...
		foreach ($data['data'] as $r) {
			...
			$rows[$i]['tenor_txt'] = $nama_simpanan->tenor;
			if($r->tenor == '0'){
				$tenor = '-';
			}
			else{
			...
			$rows[$i]['jumlah_txt'] = number_format($r->jumlah,2,',','.');
	...
	function cetak_laporan() {
		...
		foreach ($simpanan as $row) {
			...
			<tr>
				...
				<td class="h_kanan"> '.number_format($row->jumlah,2,',','.').'</td>
				<td> '.$row->user_name.'</td>
			</tr>';
		}
		$html .= '
		<tr>
			...
			<td class="h_kanan"> <strong>'.number_format($jml_simpanan,2,',','.').'</strong></td>
		</tr>
		</table>';
		$pdf->nsi_html($html);
					...
					foreach ($data_kolom as $kolom => $val) {
						...
						if ($kolom == B) {
							...
							if($query->num_rows()>0){
								...
								$data_list[$no][$kolom] = $val;
								$val2 = '<font color="red">Anggota : '.$val.' tidak di temukan dalam master Anggota dan '.$val.' tidak akan di upload </font>';
							}
						}  else if ($kolom == G) {
							if ($val2 != "") {
								$data_list[$no][$kolom] = $val2;
							} else {
								$data_list[$no][$kolom] = $val;
							}
						} else
