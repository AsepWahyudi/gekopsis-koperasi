...
class Transfer_kas extends OperatorController {
	...
	public function index() {
		...
		$this->data['judul_sub'] = 'Transfer Kas <a href="'.site_url('transfer_kas/import').'" class="btn btn-sm btn-success">Import Data</a>';
	...
	function import() {
		...
		$this->data['judul_sub'] = 'Transfer Kas <a href="'.site_url('transfer_kas').'" class="btn btn-sm btn-success">Kembali</a>';
		...
		if($this->input->post('submit')) {
			...
			if ( ! $this->upload->do_upload('import_transfer_kas')) {
		...
		$this->data['isi'] = $this->load->view('transfer_kas_import_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);
	}
	
	function import_db() {
		if($this->input->post('submit')) {
			$this->load->model('transfer_m','transfer', TRUE);
			$data_import = $this->input->post('val_arr');
			if($this->transfer->import_db($data_import)) {
	...
	function export_excel(){
		...
		$data   = $this->transfer_m->get_data_excel();
		$i	= 0;
		$rows   = array(); 
