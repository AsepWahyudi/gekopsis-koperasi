...
function get_ags_ke($master_id) {
	...
	$jml_denda_det = number_format($denda ->total_denda * 1,2,',','.');
	//sudah dibayar
	$dibayar=$this->general_m->get_jml_bayar($master_id);
	$sudah_bayar_det = number_format($dibayar ->total2,',','.');
	//sisa tagihan
	$tagihan_det = $row_pinjam->ags_per_bulan * $row_pinjam->lama_angsuran;
	$sisa_tagihan_det = number_format($tagihan_det - $dibayar ->total2,',','.');
	$sisa_tagihan_num = ($tagihan_det - $dibayar ->total);
	$sisa_tagihan = number_format($sisa_tagihan_num2,',','.');
	//sisa angsuran
	$sisa_ags_det = $row_pinjam->lama_angsuran - ($ags_ke - 1) ;
	//total pembayaran + denda
	$total_bayar_det = number_format($sisa_tagihan_num + $jml_denda_num2,',','.');
	...
	if($tgl_bayar != '') {
		...
		if ( $tgl_toleransi > $denda_hari ) { // 20140615 - 20140600
			...
			$denda_semua = '' . number_format($denda_semua_num2,',','.');
		}
	}

	// total tagihan
	$total_tagihan = number_format($sisa_tagihan_num + $jml_denda_num + $denda_semua_num2,',','.');
	if($ags_ke > $lama_ags) {
		$data = array(
			'ags_ke' 				=> 0,
			'sisa_ags' 				=> $sisa_ags,
