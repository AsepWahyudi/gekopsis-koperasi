...
class neraca_skonto extends AdminController {
	...
	public function index() {
		...
		$this->data['judul_sub'] = 'Neraca Skonto <a href="'.site_url('neraca_skonto/import').'" class="btn btn-sm btn-success">Import Data</a>';

		$this->output->set_template('gc');

		$this->load->library('grocery_CRUD');
		$crud = new grocery_CRUD();
		$crud->set_table('neraca_skonto');
		$crud->set_subject('Neraca Skonto');
	
		$crud->fields('kelompok_akunid_debet','jns_akun_id_debet','is_total_debet','kelompok_akunid_kredit','jns_akun_id_kredit','is_total_kredit','no_urut');		
    $crud->required_fields('no_urut');
    $crud->field_type('is_total_debet','true_false');
    $crud->field_type('is_total_kredit','true_false');

		$crud->set_relation('kelompok_akunid_debet','kelompok_akun','nama_kelompok');
		$crud->set_relation('kelompok_akunid_kredit','kelompok_akun','nama_kelompok');
		$crud->set_relation('jns_akun_id_debet','jns_akun','{no_akun} - {nama_akun}');
		$crud->set_relation('jns_akun_id_kredit','jns_akun','{no_akun} - {nama_akun}');

		$crud->display_as('kelompok_akunid_debet','Kelompok Akun (Debet)');
		$crud->display_as('kelompok_akunid_kredit','Kelompok Akun (Kredit)');
		$crud->display_as('jns_akun_id_debet','Jenis Akun (Debet)');
		$crud->display_as('jns_akun_id_kredit','Jenis Akun (Kredit)');
		$crud->display_as('is_total_debet','Total (Debet) ?');
		$crud->display_as('is_total_kredit','Total (Kredit) ?');
