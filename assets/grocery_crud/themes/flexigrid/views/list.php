		...
		<thead>
			<tr class='hDiv'>
				<?php foreach($columns as $column){
					$kolom_baru = $column_width;
					if($column->field_name == 'no_urut') {
						$kolom_baru = "5";
					}
				?>
				<th width = '<?php echo $kolom_baru;?>%'>
...
<?php foreach($list as $num_row => $row){ ?>        
		<tr  <?php if($num_row % 2 == 1){?>class="erow"<?php }?>>
			...
			?>
			<td width = '<?php echo $kolom_baru;?>%' class='<?php if(isset($order_by[0]) &&  $column->field_name == $order_by[0]){?>sorted<?php }?>'>
